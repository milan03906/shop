(() => {
  ...
  function pickSafeImage(p, id) {
    ...
    const candidates = [...imgs, p?.image, p?.thumbnail]
      .filter(Boolean)
      .map((x) => String(x).trim());
  ...
  function normalizeProduct(p, idx) {
    ...
    const discountPercent = Math.max(
      0,
      Math.min(100, toNumber(p?.discountPercentage, 0))
    );
    ...
    try {
      for (let page = 0; page < maxPages; page++) {
        ...
        for (let i = 0; i < arr.length; i++) {
          const product = normalizeProduct(arr[i], i);
